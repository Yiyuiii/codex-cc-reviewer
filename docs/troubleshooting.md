...
- Cache fields look contradictory: `cache.inputTokens` is residual uncached input, not total input. `cache.effective: "disabled"` only means this request did not ask for the 1-hour cache hint; raw fields can still show 1-hour or 5-minute cache activity reported by Claude Code.
